 ...
 * @param {boolean} [options.promoteLongs=true] Convert Long values from the studdb into Numbers if they fit into 53 bits
 ...
 * @param {string} db The studdb we are invoking the mechanism against
...
// Add the new credential for a studdb, removing the old
// credential from the cache
var addCredentials = function(state, db, argsWithoutCallback) {
  // Remove any credentials for the studdb
...
var clearCredentials = function(state, ns) {
  ...
  // Filter out all credentials for the studdb the user is logging out off
  for(var i = 0; i < state.credentials.length; i++) {
