 ...
 * MongoClient.connect(url, function(err, studdb) {
 *   // Get an additional studdb
 *   var testDb = studdb.studdb('test');
 *   studdb.close();
 * });
 */
 ...
 * @property {(Server|ReplSet|Mongos)} serverConfig Get the current studdb topology.
 * @property {number} bufferMaxEntries Current bufferMaxEntries value for the database
 * @property {string} databaseName The name of the database this instance represents.
 * @property {object} options The options associated with the studdb instance.
 * @property {boolean} native_parser The current value of the parameter native_parser.
 * @property {boolean} slaveOk The current slaveOk value for the studdb instance.
...
var Db = function(databaseName, topology, options) {
  ...
  // var self = this;  // Internal state of the studdb object
  this.s = {
    // Database name
  ...
  // Ensure we have a valid studdb name
  ...
  // This is a child studdb, do not register any listeners
...
/**
 * Close the studdb and it's underlying connections
 * @method
...
Db.prototype.close = function(force, callback) {
  ...
  if(this.listeners('close').length > 0) {
    ...
    // If it's the top level studdb emit close on all children
...
/**
 * Return the Admin studdb instance
 * @method
 * @return {Admin} return the new Admin studdb instance
 */
 ...
 * can use it without a callback in the following way. var collection = studdb.collection('mycollection');
 *
 * @method
...
/**
 * Get the list of all collection information for the specified studdb.
 *
 * @method
...
Db.prototype.listCollections = function(filter, options) {
  ...
  if(!this.serverConfig.capabilities().hasListCollectionsCommand) {
    // If we have legacy mode and have not provided a full studdb name filter it
 ...
 * @param {MongoError} error An error instance representing the error during the execution.
 * @param {Collection[]} collections An array of all the collections objects for the studdb instance.
 */
...
/**
 * Fetch all collections for the current studdb.
 *
 * @method
...
/**
 * Creates an index on the studdb and collection collection.
 * @method
...
/**
 * Create a new Db instance sharing the current socket connections. Be aware that the new studdb instances are
 * related in a parent-child relationship to the original instance so that events are correctly emitted on child
 * studdb instances. Child studdb instances are cached so performing studdb('db1') twice will return the same instance.
 * You can control these behaviors with the options noListener and returnNonCachedInstance.
 *
 * @method
 * @param {string} name The name of the database we want to use.
 * @param {object} [options=null] Optional settings.
 * @param {boolean} [options.noListener=false] Do not make the studdb an event listener to the original connection.
...
Db.prototype.db = function(dbName, options) {
  ...
  // Do we have the studdb in the cache already
  if(this.s.dbCache[dbName] && options.returnNonCachedInstance !== true) {
    return this.s.dbCache[dbName];
  }

  // Add current studdb as parentDb
...
var _executeAuthCreateUserCommand = function(self, username, password, options, callback) {
  ...
  if(options.digestPassword != null) {
    throw toError("The digestPassword option is not supported via add_user. Please use studdb.command('createUser', ...) instead for this option.");
  }
  ...
  // Check the studdb name and add roles if needed
...
var authenticate = function(self, username, password, options, callback) {
  // the default studdb to authenticate against is 'self'
...
Db.prototype.logout = function(options, callback) {
  ...
  // We authenticated against a different studdb use that
...
var createListener = function(self, e, object) {
  var listener = function(err) {
    if(e != 'error') {
      ...
      // Emit on all associated studdb's if available
